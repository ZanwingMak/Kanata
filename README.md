...
Kanata 1.0.0 支持 iPhone、iPad 与 Apple TV。公开测试地址：[加入 TestFlight 测试](https://testflight.apple.com/join/YbbZrdgv)

新安装默认不显示公开平台弹幕来源与 B 站登录入口。需要测试完整功能时，完成以下任意一种操作即可解锁，成功后应用会显示“功能已解锁”：

1. 在“设置”中连续点击版本号 15 次。
2. 添加媒体源时选择“网络直链 / HLS”，在名称或地址任意位置输入 `kanata`，然后加入媒体库。
3. 在“设置 → 扩展弹幕网关”中，将网关地址填写为 `kanata` 并点击“测试连接”。
...
完整功能解锁后，Apple App 可以直接使用公开平台弹幕来源，也可以连接自托管网关。网关不是播放代理，不接收或转存用户的视频文件。
